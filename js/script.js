...
(function($) {
    ...
    $(window).scroll(function () {
        ...
        if (onboardingDivBottom < document.documentElement.clientHeight) {
          ...
          if (onboardingDivBottom < document.documentElement.clientHeight - 70) {
            $(".onboarding-button").removeClass("button-transparent onboarding-button-big");
            $(".onboarding-button").addClass("button-blue onboarding-button-small");
            ...
            if (distanceFromBottom <= footerDivHeight - distanceFromBottom + 350) {
              ...
              if (distanceFromBottom <= footerDivHeight - distanceFromBottom + 50) {            
                $(".onboarding-button").removeClass("onboarding-button-small");    
                $(".onboarding-button-text").css({
                  "display":"inline",
                });
                $(".onboarding-button").css({
          ...
          $(".onboarding-button").removeClass("button-blue onboarding-button-small");
          $(".onboarding-button").addClass("button-transparent onboarding-button-big");
          $(".onboarding-button-text").css({
            "display":"inline",
          })
        }
  ...
  if ($('.route-apic-appcreate .main-container .row, .route-apic-appcreate-step-two .main-container .row, .route-applicationcreate .main-container .row, .route-applicationcreate-step-two .main-container .row, .route-apic-appverify-clientsecret .main-container .row, .route-apic-appcredentials-create .main-container .row, .route-apic-appreset-clientid .main-container .row, .route-apic-appreset-clientsecret .main-container .row, .route-apic-appcredentials-update .main-container .row, .route-apic-appcredentials-delete .main-container .row, .route-apic-appupdate .main-container .row, .route-apic-appdelete .main-container .row, .route-apic-apppromote .main-container .row, .route-apic-appimage-upload .main-container .row, .route-apic-appimage-delete .main-container .row, .route-consumerorgcreate .main-container .row, .route-consumerorgupdate .main-container .row, .route-consumerorgdelete .main-container .row, .route-consumerorgchangeowner .main-container .row, .route-consumerorginvite .main-container .row, .route-consumerorgremove-user .main-container .row, .route-consumerorgresend-invite .main-container .row, .route-change-pwd-pagechange-password-form .main-container .row .apic-change-pwd-form, .route-entityuseredit-form .main-container .row .user-form, .route-ibm-apimsubscription-wizardnoplan .modal-dialog .form-item input, form.user-pass-reset, .apicForm, .apic-user-form, .consumerorg-change-member-role-form').length === 1) {
    ...
    // if ($('.consumerorg-invite-user-form').length) {
    //   $('.page-header').text('Приглашение участника');
    //   $('<p class="invite-user-text">Отправьте пользователю приглашение присоединиться к клиентской организации</p>').insertAfter('.page-header');
    // }
  } else {
    if ($(window).width() < 768) {
      $('.page-header').css({
  ...
  //   </span>
  // `);
  // if ($('.apicNewUserWrapper')) {
  //   $('.orgInvite').html(inviteMemberSpan);
  // }
  ...
  if ($('.apicMyOrgMembers').length) {
    $('.organizationMembers').append(scrollRight('my-org'));
  }
  if ($('.client-path-bg').length) {
    $('.client-path-bg').append(scrollRight('client-path-arrow'));
    $('.news').append(scrollRight('news-scroll'));
  }
  if ($('.subscriptionsTable').length && $('.noSubsFound').length === 0) {
    $('.subscriptionsTable').append(scrollRight('subscriptions'));
  }


  Drupal.behaviors.mainVtbBehaviors = {
    attach: function (context, settings) {

      // Замена стрелки дропменю в шапке профиля и добавление такой же стрелки
      // в раздел поддержки FAQ
      ...
      if ($(".dropit-icon").length) {
        $(".orgmenu.dropit > .dropit-trigger > a > .dropit-icon").once('dropit-icon').html(dropitArrowCreate('white'));
      }
      if ($(".faqfield-question").length) {
        $(".faqfield-question").once('faqfield-question').append(dropitArrowCreate('#7B7E86'));
      }
      // Замена стрелок в селектах
      if ($('.bx--select__arrow').length) {
        $('.bx--select__arrow').remove();
        $('.bx--select').once('bx--select__arrow').append(dropitArrowCreate('#7B7E86', 'bx--select__arrow'));
      }
    }
  };
  ...
  if ($('.definitionsBody').length) {
    $('.definitionsBody > div > table > thead > tr > th')[0].remove()
    const definitionsArray = $("tr.definitionRow");
    definitionsArray.each((i, definition) => {
      const arrayTd = $(definition).find('.bx--table-expand-v2');
      arrayTd.appendTo(definition)
    });
  }

  const spinningArrowsSvg = $(`
  ...
  if ($('.reload-captcha').length) {
    $('.reload-captcha').prepend(spinningArrowsSvg);
  }
  ...
  if ($( ".swaggerDownload" ).length) {
    $( ".swaggerDownload" ).html(downloadIconSvg);

    // Захват ссылки на скачивание апи и других атрибутов
    const href = $('.swaggerDownload').attr('href');
    const downloadAttr = $('.swaggerDownload').attr('download');
    ...
    const divDownload = $('.operationDownloadOpenApi');
    $(divDownload).find('.labelText').text('Скачать Swagger-файл')
    buttonDownload.innerHTML = divDownload.html();
    // Замена div на ссылку
    divDownload.replaceWith($(buttonDownload));
  }
  ...
  if ($( "button.close" ).length) {
    $( "button.close > span" ).html(crossSvg)
  }

  // Изменение высоты textarea в разделе поддержки
  if ($(".form-textarea").length) {
    $( ".form-textarea").attr('rows', '6');
  }

  // Работа с капчей
  // if ($('.form-item-captcha-response label').length) {
  //   $('.form-item-captcha-response label').text('Введите код с картинки');

  //   $('.reload-captcha-wrapper a').text('Обновить картинку');
  //   $('.reload-captcha-wrapper').insertAfter($('[data-drupal-selector="edit-captcha-image"]'));
  // }
  ...
  `)
  if ($('.connection-section-title-icon').length) {
    const iconContainers = $('.connection-section-title-icon');
    $(iconContainers[0]).html(icon1);
    $(iconContainers[1]).html(icon2);
  }
  ...
  // if ($('.view-filter-button').length) {
  //   $('.view-filter-button').html(filterButton)
  // }
  ...
  if ($('.apicMyOrg').length) {
    // Владелец
    const ownerMail = $('.apicMyOrgOwner .apicMyOrgMemberDetails').find('.apicUserMail').text();
    $('.apicMyOrgOwner .apicMyOrgMemberDetails').find('.ownerTag').remove();
    $(`<div><p>${ownerMail}</p></div><div><p>Владелец</p></div>`).insertAfter('.apicMyOrgOwner .apicMyOrgMemberDetails .orgApicUser');
    $('.apicMyOrgOwner .apicMyOrgMemberDetails').find('.apicUserMail').remove();


    if ($('.apicMyOrgNoMembers').length) {

    } else {
      // Реорганизация таблицы
      // Участники
      const membersArray = $("tr[class^='member']");
      membersArray.each((i, member) => {
        // Захват роли
        const tdWithRole = $(member).find('td > span.apicMyOrgMemberRoleEnabled');
        ...
        // Захват почты
        const emailSpan = $(member).find('.apicUserMail');
        // Добавление роли
        $( `<td></td><td><p>${emailSpan.text()}</p></td><td><p>${roleName}</p></td>` ).insertAfter(tds[0]);
        // Удаление спана с имейлом
        emailSpan.remove()
      });
    }
  }
  if ($('.apicMyOrg').length && $('.apicMyOrgNoMembers').length === 0) {
  }

  // Добавление слова "ver." на страницу документации АПИ
  if ($('.apiconnect-explorer').length) {
    $('.apicProductVersion.apiVersion').prepend('ver. ')
  }
  ...
  } else if ($('.application-view').length || $('.applicationWrapper').length || $('.applicationWrapper').length) {
    $('.header-menu-links-item:contains("Приложения")').addClass('header-menu-links-item-active')
  } else if ($('.ibmapimSupport').length) {
    $('.header-menu-links-item:contains("Поддержка")').addClass('header-menu-links-item-active')
